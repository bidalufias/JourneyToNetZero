...
await phones.government.locator('.sheet__body .ocard:not([disabled])').first().click(); await wait(300)
...
await phones.business.locator('.sheet__body .ocard:not([disabled])').first().click(); await wait(400)
...
await phones.activist.getByRole('button', { name: 'TURN THE SPOTLIGHT ON' }).click(); await wait(400)
...
await phones.government.locator('.sheet__body .ocard:not([disabled])').first().click(); await wait(200)
...
await phones.community.getByRole('button', { name: /YES\. SAY NO/ }).click(); await wait(400)
...
// Two seats say they are done: the masthead and the seat row count them.
await phones.government.getByRole('button', { name: 'I AM DONE TALKING' }).click(); await wait(200)
await phones.business.getByRole('button', { name: 'I AM DONE TALKING' }).click(); await wait(300)
await shot(dash, 'dash-r1-table-two-done')
await shot(phones.government, 'phone-r1-table-done', true)
...
await wait(4500); await shot(dash, 'dash-r1-reckoning-4s')
await shot(phones.government, 'phone-r1-reveal-mirror-1', true)
...
await shot(phones.community, 'phone-r1-reveal-mirror-4', true)
